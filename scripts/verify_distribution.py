...
_STANDARD_LICENSE_FILE = re.compile(
    r"^(?:authors|copying|license|notice)(?:\.(?:md|rst|txt))?$",
    re.IGNORECASE,
)
...
def _wheel_member_allowed(parts: Sequence[str]) -> bool:
    ...
    return (
        ...
        and _STANDARD_LICENSE_FILE.fullmatch(parts[2]) is not None
    )
...
def _verify_distribution_in_temporary(
    ...
    if not (
        sentinel.get("status") == "drift_detected"
        and sentinel.get("authority") == "diagnostic_only"
        and sentinel.get("apply_blocking") is False
    ):
        raise DistributionVerificationError(
            "installed_contract_spine_unexpected_isolated_result"
        )
    supported_result = _run(
        [
            ...
            (
                "import json, sys; "
                "from hsconfig.contract_spine_sentinel import "
                "build_contract_spine_sentinel_report; "
                "print(json.dumps(build_contract_spine_sentinel_report(sys.argv[1]), "
                "sort_keys=True))"
            ),
            str(source_root),
        ],
    ...
    supported = json.loads(supported_result.stdout)
