...
3. Prevent silent order-sensitive changes at new normal candidate admission. Preserve historical serialization/intent and accepted-package validation; reject authored priorities that existing serialization would change. Reject ambiguous overlapping different-value GlobalValues permutations and hidden order-only baseline drift, using actionable revision feedback. Disjoint conditions remain usable. This avoids a new schema or retroactive invalidation.
4. Behaviorally verify the existing independent reviewer for source freshness and exact-list relevance, evidence versus inference, all 38 GlobalValues decisions, magnitude justification, actual emitted order, and critical decisions that are encoded, delegated, or unsupported. Strengthen instructions only for demonstrated omissions; describe new admission restrictions concisely. No new schema, review framework, or candidate tournament.
