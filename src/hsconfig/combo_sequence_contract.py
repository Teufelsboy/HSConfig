...
from hsconfig.package_domain import ComboTiming


SUPPORTED_TIMING_TO_OPERATOR = MappingProxyType(
    {timing.value: timing.operator for timing in ComboTiming}
)


def build_combo_sequence_contract(
    claim: dict[str, Any],
    deck_cards: set[str],
) -> dict[str, Any]:
    sequence = [
        str(card)
        for card in claim.get("sequence", claim.get("cards", []))
        if str(card)
    ]
    ...
    timing = ComboTiming(timing_kind)
    operator = str(claim.get("operator", timing.operator))
    if operator != SUPPORTED_TIMING_TO_OPERATOR[timing_kind]:
        return {
            "emittable": False,
            "reason": "operator_timing_mismatch",
            "cards": sequence,
        }
