...
from hsconfig.io import file_sha256
...
def package_fingerprint(package_root: str | Path) -> dict[str, Any]:
    ...
    for path in sorted(package.rglob("*")):
        ...
        rows.append(
            {"path": relative, "size": size, "sha256": path_hash}
        )
...
def runtime_snapshot(
    ...
    return {
        ...
        "deck_config_ini_sha256": (
            file_sha256(deck_config) if deck_config.exists() else None
        ),
    }
...
def build_fake_apply_receipt(
    *,
    ...
    return {
        ...
        "runtime_snapshot_before": runtime_snapshot(runtime, config_dir),
