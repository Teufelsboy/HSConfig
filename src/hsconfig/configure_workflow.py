...
from hsconfig.commands.apply import (
    apply_payload as default_apply_payload,
    validate_payload,
)
...
from hsconfig.config_quality_contract import (
    build_config_quality_report as default_build_config_quality_report,
...
from hsconfig.configure_summary import (
...
SourceAcquireForConfigure = Callable[
    [argparse.Namespace],
...
def execute_configure(
    ...
    *,
    ...
    configure_fault_hook: ConfigureFaultHook | None = None,
    apply_payload_fn: ApplyPayload = default_apply_payload,
    build_config_quality_report_fn: BuildConfigQualityReport = (
        default_build_config_quality_report
    ),
    source_manifest_payload_fn: SourceManifestPayload = source_manifest_payload,
    source_acquire_for_configure_fn: SourceAcquireForConfigure = (
        source_acquire_for_configure
    ),
) -> ConfigureResult:
...
def _request_namespace(request: ConfigureRequest) -> argparse.Namespace:
    return SimpleNamespace(
        ...
        source_evidence_json=_path_text(request.source_evidence_json),
        auto_source=request.auto_source,
        online_source=request.online_source,
        source_search_results_json=_path_text(
            request.source_search_results_json
        ),
        source_url=list(request.source_urls),
        source_fixture_url_map_json=_path_text(
            request.source_fixture_url_map_json
        ),
        source_fetch_timeout_seconds=request.source_fetch_timeout_seconds,
        current_date=request.current_date,
        cards_json=_path_text(request.cards_json),
        collectible_cards_json=_path_text(
            request.collectible_cards_json
        ),
...
def _execute_configure_namespace(
    ...
    *,
    ...
    package_model_observer: PackageModelObserver | None = None,
) -> tuple[dict[str, Any], int]:
    ...
    try:
        ...
        manifest_payload, manifest_status = source_manifest_payload_fn(
    ...
    if bool(getattr(args, "online_source", False)):
        ...
        try:
            ...
            ) = source_acquire_for_configure_fn(
                acquisition_args
            )
        except Exception as exc:
    ...
    try:
        ...
        prepare_payload, prepare_status = prepare_package_payload(
            ...
            stage_observer=stage_observer,
            model_observer=package_model_observer,
    ...
    if bool(getattr(args, "apply", False)):
        try:
            ...
            return _finish_stage_exception(
                out,
                ConfigureFaultPoint.APPLY.value,
                exc,
            )
        deck_input_verification = operator_summary.get(
            "deck_input_verification",
    ...
    try:
        ...
        return _finish_stage_exception(
            out,
            ConfigureFaultPoint.CONFIGURE_SUMMARY.value,
            exc,
        )
    return _finish(out, "OK", summary_payload, 0)
...
def _build_config_quality_summary(
    package_dir: Path,
    *,
    build_config_quality_report_fn: BuildConfigQualityReport = (
        default_build_config_quality_report
    ...
    try:
        return _compact_config_quality_summary(
            build_config_quality_report_fn(package_dir)
        )
    except Exception as exc:
        summary = {
            "status": "attention",
