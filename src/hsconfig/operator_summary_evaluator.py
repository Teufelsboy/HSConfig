...
from hsconfig.operator_summary_inputs import thaw_operator_value
...
def _build_operator_summary_unfrozen(
    *,
    ...
    source_backed_strong_closure = _source_backed_strong_closure(
        source_status_resolution=source_status_resolution,
        default_only_runtime_surfaces=default_only_runtime_surfaces,
...
def _source_backed_strong_closure(
    *,
    source_status_resolution: SourceStatusResolution,
...
def _guide_strength_summary(
    *,
    ...
    return {
        ...
        "source_backed_strong_requires": list(
            SOURCE_BACKED_STRONG_REQUIREMENTS
        ),
    }
...
def _invoke_operator_summary_owner_factory(
    integrity_bootstrap: Any,
    evaluator: Any,
    thaw: Any,
) -> tuple[Any, Any]:
    return integrity_bootstrap("freeze", (evaluator, thaw))


(
    _owner_evaluator_primary,
    _owner_thaw_primary,
) = _operator_integrity_bootstrap(
    "freeze",
    (_build_operator_summary_unfrozen, thaw_operator_value),
)
(
    _owner_evaluator_backup,
    _owner_thaw_backup,
) = _operator_integrity_bootstrap(
    "freeze",
    (_build_operator_summary_unfrozen, thaw_operator_value),
)
(_owner_invoker_primary,) = _operator_integrity_bootstrap(
    "freeze",
    (_invoke_operator_summary_owner_factory,),
)
(_owner_invoker_backup,) = _operator_integrity_bootstrap(
    "freeze",
    (_invoke_operator_summary_owner_factory,),
)
_operator_summary_evaluator_bootstrap = _operator_integrity_bootstrap(
    "guard",
    primary=_owner_invoker_primary,
    backup=_owner_invoker_backup,
    primary_bound=(
        _operator_integrity_bootstrap,
        _owner_evaluator_primary,
        _owner_thaw_primary,
    ...
    name="_operator_summary_evaluator_bootstrap_factory",
)
del (
    _owner_evaluator_primary,
    _owner_thaw_primary,
    _owner_evaluator_backup,
    _owner_thaw_backup,
    _owner_invoker_primary,
    _owner_invoker_backup,
)


def _protected_evaluator_module_type(
    capability: Any,
) -> type[ModuleType]:
    class _ProtectedEvaluatorModuleType(ModuleType):
        def __getattribute__(self, name: str) -> Any:
            if name == "_operator_summary_evaluator_bootstrap":
                return capability
        ...
        def __setattr__(self, name: str, value: Any) -> None:
            if name in {
                ...
                raise AttributeError(
                    f"protected_operator_evaluator_binding:{name}"
                )
    ...
    return _ProtectedEvaluatorModuleType


_ProtectedEvaluatorModule = _protected_evaluator_module_type(
    _operator_summary_evaluator_bootstrap
)
_evaluator_module = sys.modules[__name__]
_evaluator_module.__class__ = _ProtectedEvaluatorModule
del _evaluator_module.__dict__["_operator_summary_evaluator_bootstrap"]
del _evaluator_module
