...
def test_dossier_does_not_promote_nested_malformed_default_only_surface(
    tmp_path: Path,
) -> None:
    package_dir = _package(tmp_path)
    research_path = tmp_path / "matching_nested_default_only_strong.json"
    write_json(
        research_path,
        {
            "deck_name": "ShadowPriest",
            ...
            "first_missing_source_action": "none",
            "notes": "Nested malformed default-only data must not promote.",
        },
    )
