...
IDENTITY_FIELDS = {"deck_code", "hs_id", "hdt_deck_id"}
EXPECTED_DECKS = {
    "ShadowPriest",
    "CtAPaladin",
    "PirateRogue",
...
def test_archetype_fixture_matrix_has_actionable_rows():
    for row in _matrix()["decks"]:
        ...
        assert (
            row.get("first_missing_source_action", "none")
...
def test_archetype_fixture_matrix_references_identity_catalog():
    matrix = _matrix()

    assert matrix["identity_catalog"] == "audited-deck-catalog.json"
    for row in matrix["decks"]:
        assert IDENTITY_FIELDS.isdisjoint(row)
...
def test_each_fixture_row_documents_decision_family_and_limits():
    ...
    for deck in matrix["decks"]:
        ...
        assert all(
            isinstance(item, str) and item for item in deck["known_coverage_limits"]
        )
