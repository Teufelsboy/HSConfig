...
Normal HSConfig output is limited to the files above. `Concede.json` and `Presume.json` are documented HearthRanger surfaces, but normal HSConfig does not emit them; absence never blocks a valid load-safe package.
