...
- `globalvalue_numeric_tuning` is report-visible and runtime-evidence-required; use `gameplan_posture` for Step1 GlobalValues posture that may lower only with the canonical non-plan source-document receipt binding the normalized claim to decoded exact-deck evidence and the target fingerprint. `--claims-json`, imported lifecycle claims, and plan reports cannot manufacture that receipt; any explicit official/static/statistical/non-guide provenance signal vetoes it, and plan GlobalValues rows are rebuilt from the receipt rather than accepted by Claim ID.
