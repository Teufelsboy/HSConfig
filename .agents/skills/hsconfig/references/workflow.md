...
Normal workflow: prefer `hsconfig configure ...`; read `<out>/configure_summary.json.acceptance_summary` first after configure, then `<out>/configure_summary.json.handoff_contract` for compact diagnostic-only handoff proof, then `<out>/configure_summary.json.source_closure_receipt` when source depth is the question; use lower-level commands only when inspecting a stage (`source-manifest -> source-autopilot or draft-source-documents -> research-deck -> prepare -> validate -> apply`); use `reports/operator_summary.json` as the apply authority. It is the sole human-facing verdict; never infer apply readiness from individual reports. The source receipt is diagnostic-only, cannot promote, block, apply, or write runtime files, and keeps source_status_apply_blocking=false.
