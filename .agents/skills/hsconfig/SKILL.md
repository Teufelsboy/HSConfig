...
- Treat source claim kind and runtime surface authority as separate decisions.
  ...
  reports.
- When source coverage is weak, still build the package; report load-safe or partial, and never label policy-backed/default-only runtime rows as `SOURCE_BACKED_STRONG`. For start-of-game enablers such as Darkbishop Benedictus, separate effect semantics from opening-hand mulligan keep behavior.
