...
- The audited twelve-deck acceptance is read-only: it requires exactly eleven unique representative manifest rows plus exactly one supplemental CuteWarrior row, without promoting that supplemental row; captured/fixture provenance remains `diagnostic_source_not_apply_eligible`, while only a separately acquired exact `live_http` / `live_verified` package that passes current strict validation may be technically eligible. Preserve semantic-enrichment proof that neither source nor linked runtime owner is a spell for `OnBoardBonus` or `BeforeBattlecryTargetBonus`, typed duplicate-preserving physical/report parity plus claim/ref provenance, suppressed unsupported conditions, and the named ShadowPriest, MechPala, Kingslayer/Boarlock, Discolock, and ImbueMage boundaries. The test uses a build-pinned, upstream- and snapshot-digest-checked exact DBF set, denies global and directly imported Cardfeed/DNS/socket paths plus runtime writes, and does not prove in-client execution, gameplay improvement, matchup quality, or optimality.
## Source Contract
