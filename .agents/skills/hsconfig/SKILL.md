...
- If no source-backed Mulligan keep can be emitted, allow `policy_backed_autonomous_mulligan` to emit a small low-curve keep set; it prevents default-only output, stays weaker than guide evidence, vetoes cards with explicit/suppressed/quarantined Mulligan source intent, and must not keep non-hand start-of-game enablers such as Darkbishop Benedictus without explicit opening-hand source text.
