...
- Strong promotion may use explicit `deck_matched_public_guide`, explicit `archetype_matched_public_guide`, and explicit `official_static_semantics` only when the claim matches the runtime surface. `decklist_only`, decklist-only pages, statistical enrichment, `policy_fallback`, snippets, `default_runtime`, and runtime examples do not count as strong evidence.
